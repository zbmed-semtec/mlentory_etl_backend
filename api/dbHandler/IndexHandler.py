...
from api.entities.ElasticsearchEntities import HFModel, Model, OpenMLRun, Run, AI4LifeModel
...
class IndexHandler:
    def __init__(self, es_host="localhost", es_port=9200):
        ...
        self.es = Elasticsearch(
            ...
            request_timeout=30,  # Increase from default 10s to 60s
    ...
    def create_openml_index_entity_with_dict(self, info:Dict, uri:str):
        ...
        index_run_entity.platform = "OpenML"
        index_run_entity.dateCreated = datetime.datetime.now()
        ...
        for key, value in info.items():
            if "identifier" in key:
                ...
                index_run_entity.dateCreated = value[0]
    ...
    def create_ai4life_index_entity_with_dict(self, info:Dict, uri:str):
        ...
        index_model_entity.db_identifier = uri
        index_model_entity.name = ""
    ...
    def create_model_index_entity_with_dict(self, info: Dict, model_uri: str, platform: str):
        ...
        index_model_entity.sharedBy = ""
        index_model_entity.releaseNotes = ""
        index_model_entity.license = "Information not found"
        index_model_entity.mlTask = []
        index_model_entity.keywords = []
        index_model_entity.relatedDatasets = set()
        index_model_entity.baseModels = []
    ...
    def search_complete_response(self, index_name, query):
        try:
            result = self.es.search(index=index_name, body=query)
            return result
        except Exception as e:
            print(f"Error searching index: {str(e)}")
            # Return empty response with expected structure instead of empty list
            return {
                "hits": {"total": {"value": 0, "relation": "eq"}, "hits": []},
                "aggregations": {}
            }

    def open_pit(self, index_name, keep_alive="1m"):
        try:
            response = self.es.open_point_in_time(index=index_name, keep_alive=keep_alive)
            return response["id"]
        except Exception as e:
            print(f"Error opening PIT: {e}")
            return None
